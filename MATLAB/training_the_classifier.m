...
for numberOfFilesToRead = 1:10
    for string = stringOptions
        for fret = fretCounter
            ...
            x(:,string,fret) = icassp19_hilbert_transform(sig);
...
for string = stringOptions 
    for fret = fretCounter
        ...
        gamma(:,:,string,fret) = cov([muPitch(:,string,fret) muB(:,string,fret)])
...
for string = stringOptions 
    for fret = fretCounter      
        tmp1 = muPitch(:,:,fret);
        tmp2 = muB(:,:,fret) ;
        tmp3(string) = mean(diag(gamma(:,:,string,fret)));
        mu = [tmp1(:) tmp2(:)];
        sigma = mean(tmp3);
        % labels ?
        labels = [(1:length(mu))'];
...
for string = stringOptions 
    for fret = fretCounter
        ...
        outputFileName = strcat('/home/jmhh/repositories/guitar_string_finger_and_pluck_estimation/icassp_plucking_estimation/MATLAB/mats/trained_model_of_',guitarType,'_from_',sprintf('%1.0fth_fret',fretCounter-1),'.mat')
        save(outputFileName);
    end
    end
end
